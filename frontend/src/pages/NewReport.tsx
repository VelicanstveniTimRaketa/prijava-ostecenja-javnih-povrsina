...
function NewReport() {
  ...
  useEffect(() => {
    if (mapInputType === "myLocation") getLocation().then(setLocation);
  }, [mapInputType]);
          ...
          <Form.Item required name="lokacija" label="Lokacija" rules={[{ required: true, validator: locationValidator, message: "Molimo odaberite lokaciju." }]}>
            <div>
              ...
              <Layout style={{ margin: "1em 0" }}>
                <MapJsApi marker={location} center={center} onClick={l => setLocation({ lat: l.lat(), lng: l.lng() })} disabled={mapInputType !== "onMap"} />
              </Layout>
            </div>
          </Form.Item>
